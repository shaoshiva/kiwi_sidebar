...
?>
<div>
	<img style="display: block; float: left; width: 64px; height: 64px;" src="static/apps/kiwi_sidebar/img/64/icon.png">
	<h1 style="margin-left: 80px;"><?= strtr(__('Sidebar ‘{{title}}’'), array('{{title}}' => $sidebar->side_title)) ?></h1>
	<?php
	foreach ($sidebar->blocs as $bloc) {
		if (!empty($bloc->sibl_published) && !empty($bloc->wysiwygs->content)) {
			?>
